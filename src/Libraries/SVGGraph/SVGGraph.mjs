...
export default class SVGGraph{
  constructor({
    ...
    this.IntersectionObserver = new IntersectionObserver(function(Entries){
      ...
      root: null,
      threshold: .01, // Requires that at least 1% of the element is in the viewport
    });
    ...
    void async function Update(){
      while(!this.IsDestroyed){
        ...
        if(this.GeneratorFunction) this.AddDataPoint(this.GeneratorFunction());
      }
    }.call(this);
  ...
  ConvertNumberToText(n){
    ...
    return "" + ((Math.abs(n) < 1e-4 || Math.abs(n) >= 1e6) && Math.abs(n) > 1e-15 ? n.toExponential(5).replace(/\.([0-9]*[1-9])?0*/g, ".$1").replace(/\.e/, ".0e") : Number.parseFloat(n.toFixed(5)));
  }
  AddDataPoint(DataPoint){
    ...
    while(this.Data.length > 0){
      if(this.Data[0][0] < Now - this.HistoryLength && (this.Data.length === 1 || this.Data[1][0] < Now - this.HistoryLength)){
        this.Data.shift();
      } else{
        break;
      }
  ...
  Render(){
    ...
    for(let i = 0, CurrentY = Math.floor(MinValue / YStep) * YStep; i < 10; ++i, CurrentY += YStep){
      if(CurrentY < MinValue - MinValue * 1e-10) continue;
      if(CurrentY > MaxValue + MaxValue * 1e-10) break;
    ...
    const GraphWidth = 400 - (Math.max(20, 10 + 7 * MaxHorizontalTextLength) - 40);
    this.Wrapper.setAttributeNS(null, "transform", "translate(" + Math.max(20, 10 + 7 * MaxHorizontalTextLength) + ", 0)");
